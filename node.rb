...
when Net::HTTPOK
    ...
    $stderr.puts date + ' ' + script + 
        ": TRACE retrieving node %s from yaml/%s.yml" % [node, node]
    cat_result = `cat yaml/#{ARGV[0]}.yml 2> /dev/null || echo 0 | tr -d "\n"`
    ...
    if cat_result != '0'
        ...
        $stderr.puts date + ' ' + script + 
            ": ERROR retrieving node %s from yaml/%s.yml" % [node, node]
        $stderr.puts date + ' ' + script +
